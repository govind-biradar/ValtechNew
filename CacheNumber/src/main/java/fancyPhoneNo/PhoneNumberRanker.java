...
public class PhoneNumberRanker {

	public static List<String> rankPhoneNumber(List<String> phoneNumbers) {
        ...
        for (String phoneNumber : phoneNumbers) {
            ...
            //If all digits in the phone number are the same
            if (isAllDigitsSame(phoneNumber)) {
                score += 6;
            }

            // If the phone number is a palindrome
            if (isPalindrome(phoneNumber)) {
                score += 5;
            }

            //If the phone number has at least one repeated digit
            if (hasRepeatedDigit(phoneNumber)) {
                score += 4;
            }

            //If the phone number is in ascending order
            if (isAscendingOrder(phoneNumber)) {
                score += 3;
            }

            //If the phone number is in descending order
            if (isDescendingOrder(phoneNumber)) {
                score += 2;
            }

            //If the phone number contains a consecutive sequence of numbers
